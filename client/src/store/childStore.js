...
export const useChildStore = create((set, get) => ({
  ...
  addSession: async (childId, sessionData) => {
    try {
      set({ isLoading: true, error: null });
      const { data } = await instance.post(
      ...
      set((state) => ({
        selectedChild:
          state.selectedChild?._id === childId
            ? {
                ...state.selectedChild,
                sessions: [...state.selectedChild.sessions, data.data.session],
              }
            : state.selectedChild,
        isLoading: false,
      }));
  ...
  updateSessionStatus: async (childId, sessionId, status) => {
    try {
      set({ isLoading: true, error: null });
      ...
      set((state) => ({
        selectedChild:
          state.selectedChild?._id === childId
            ? {
                ...state.selectedChild,
                sessions: state.selectedChild.sessions.map((s) =>
                  s._id === sessionId ? data.data.session : s,
                ),
              }
            : state.selectedChild,
        isLoading: false,
  ...
  deleteSession: async (childId, sessionId) => {
    try {
      set({ isLoading: true, error: null });
      await instance.delete(`children/${childId}/sessions/${sessionId}`);
      set((state) => ({
        selectedChild:
          state.selectedChild?._id === childId
            ? {
                ...state.selectedChild,
                sessions: state.selectedChild.sessions.filter(
                  (s) => s._id !== sessionId,
                ),
              }
            : state.selectedChild,
